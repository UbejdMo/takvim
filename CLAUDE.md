...
|---|---|---|
| Sabahu / Imsaku | Fajr | Dawn |
| Lindja e Diellit | Sunrise | Not a prayer; display only; ends Fajr |
| Dreka | Dhuhr | Noon |
| Ikindia | Asr | **Use HANAFI calculation** (Kosovo is Hanafi-majority) |
...
- Times are generated by a calculation engine **calibrated to reproduce BIK to the minute**:
  Hanafi Asr, Balkan/Diyanet-style Fajr–Isha angles, plus per-prayer minute offsets (ihtiyat).
- `backend/app/data/validate_against_bik.py` must scrape BIK's published daily times
  (bislame.net/takvimi) for a sample of dates spread across the year and assert the generated
  Prishtinë times match exactly. CI/test fails on any mismatch.
- Seed once into Postgres; the app serves from the DB. **Never call an external API at request
  time.** **Never present uncalibrated/unvalidated times as official.**
